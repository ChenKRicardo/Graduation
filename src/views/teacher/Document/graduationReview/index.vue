<template>
  <div class="graduation">
    <div class="graduation-text"><h3>毕业论文审阅</h3></div>
    <el-card
      ><div class="tabel-content">
        ...
        <el-pagination
          ...
          @current-change="getGraduationList"
          :current-page="page"
          :page-sizes="[3, 5, 10]"
          :page-size="limit"
...
export default {
  ...
  methods: {
    ...
    async getGraduationList(pager = 1) {
      this.page = pager;
      const { page, limit } = this;
      try {
        let result = await this.$API.teacher.reqGetStudentGraduationInfo(
          page,
          limit
        );
        this.graduationList = result.data;
    ...
    async confirm() {
      if (this.dialogTitle) {
        ...
        try {
          await this.$API.teacher.reqPostGraduationAdvice(studentID, form);
          this.$message({ type: "success", message: "论文审核意见上传成功" });
        ...
        const { studentID, form } = this;
        try {
          await this.$API.teacher.reqUploadGraduationAdvice(studentID, form);
          this.$message({
            type: "success",
            message: "论文审核修改意见上传成功",
          });
          this.getGraduationList();
          this.dialogFormVisible = false;
        } catch (error) {
