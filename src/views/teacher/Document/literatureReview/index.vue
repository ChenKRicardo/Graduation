<template>
  <div class="literature">
    <div class="literature-text"><h3>文献综述审阅</h3></div>
    <el-card
      ><div class="tabel-content">
        ...
        <el-pagination
          ...
          @current-change="getLiteratureList"
          :current-page="page"
          :page-sizes="[3, 5, 10]"
          :page-size="limit"
...
export default {
  ...
  methods: {
    ...
    async uploadLiterature(row) {
      this.dialogFormVisible = true;
      this.dialogTitle = true;
      this.studentID = row.studentID;
    },
    async editLiterature(row) {
      this.dialogFormVisible = true;
      this.dialogTitle = false;
      this.studentID = row.studentID;
      Object.assign(this.form, row);
    },
    async getLiteratureList(pager = 1) {
      this.page = pager;
      const { page, limit } = this;
      try {
        let result = await this.$API.teacher.reqGetStudentLiteratureInfo(
          page,
          limit
        );
        this.literatureList = result.data;
    ...
    async confirm() {
      if (this.dialogTitle) {
        ...
        try {
          await this.$API.teacher.reqPostLiteratureAdvice(studentID, form);
          this.$message({
            type: "success",
            message: "文献综述审核意见上传成功",
          });
        ...
        const { studentID, form } = this;
        try {
          await this.$API.teacher.reqUploadLiteratureAdvice(studentID, form);
          this.$message({
            type: "success",
            message: "文献综述审核修改意见上传成功",
          });
          this.getLiteratureList();
          this.dialogFormVisible = false;
        } catch (error) {
